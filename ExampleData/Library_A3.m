TrueTags = {["a^0*u"],["exp(-N/-10)*a^0*u"],["a^2*u","N^1*a^2*u"]};
%% Set Boundary Trial function params
% 1. Polynomial: b(x,u) = x^p *u
BoundaryParams.polynomial.p = [1:2];
% 2. Sigmoidal:b(x,u) = 1/ (1+ exp(-k(x-x_maturity)) *u
BoundaryParams.sigmoid.x_mid =[]; 
BoundaryParams.sigmoid.k = [];
...
TransportParams.polynomial.p = [0:2];

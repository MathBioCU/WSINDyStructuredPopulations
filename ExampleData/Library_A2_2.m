TrueTags = {["a^0*u"],["exp(0.08x)*u"],["Gauss(10,5)*u"]};

%% Set Boundary Trial function params
% 1. Polynomial: b(x,u) = x^p *u
BoundaryParams.polynomial.p = [0,1,2];
% 2. Sigmoidal:b(x,u) = 1/ (1+ exp(-k(x-x_maturity)) *u
BoundaryParams.sigmoid.x_mid =[3]; %[3];
BoundaryParams.sigmoid.k = [2]; %2;
% 3. Gaussian: b(x,u) = exp( -(x-x_peak)^2/(2sigma^2)) *u
BoundaryParams.gaussian.x_peak =[11];[5:5:15]; 
BoundaryParams.gaussian.sigma = [6];
